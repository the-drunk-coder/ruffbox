...
impl<T: Eq + Copy + Hash + Ord + std::fmt::Debug> SequenceGenerator<T, usize>
    for PfaSequenceGenerator<T>
{
    fn get_next(&mut self) -> Option<T> {
...
mod tests {
    ...
    fn test_bounce_gen() {
        let mut bounce_gen =
            BounceSequenceGenerator::from_params((20.0).into(), (200.0).into(), (10.0).into());
    ...
    fn test_ramp_gen() {
        let mut ramp_gen =
            RampSequenceGenerator::from_params((20.0).into(), (200.0).into(), (10.0).into());
        let mut results = Vec::new();
        for _ in 0..10 {
